...
<button class="sub-btn" type="button" id="removeRoutes">
    ...
    function panelSlider() {
        ...
        function mobiledrag(){
          ...
          slider.ontouchmove = function dragMouseDown(e) {
              console.log(e.touches[0].clientY)
            // get position of mouse
            let dragX = e.touches[0].clientY;
            // register a mouse move listener if mouse is down
            document.ontouchmove = function onMouseMove(e) {
              // e.clientY will be the position of the mouse as it has moved a bit now
              // offsetHeight is the height of the block-1
              block.style.height = block.offsetHeight + e.touches[0].clientY - dragX + "px";
              console.log(block.offsetHeight + e.touches[0].clientY - dragX + "px")
              // update variable - till this pos, mouse movement has been handled
              dragX = e.touches[0].clientY;
              console.log(dragX)
            }
            // remove mouse-move listener on mouse-up (drag is finished now)
            document.touchend = () => document.ontouchmove = document.touchend = null;
          }
      }
    }
