...
var jsPanel = {
  ...
  ajax: function ajax(obj, ajaxConfig) {
    ...
    if (_typeof(obj) === 'object' && obj.classList.contains('jsPanel')) {
      objIsPanel = true;
    } else {
      objIsPanel = false;
    ...
    if (typeof ajaxConfig === 'string') {
      ...
      config.url = encodeURI(ajaxConfig.url); // reset timeout to 0, withCredentials & responseType to false if request is synchronous
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          config.done.call(xhr, obj); // extract and eval content of script tags if "evalscripttags"
          ...
          if (config.evalscripttags) {
            ...
            if (scripttags) {
              scripttags.forEach(function (tag) {
                ...
                var js = tag.replace(/<script\b[^>]*>/i, '').replace(/<\/script>/i, '').trim(); // execute javascript
        ...
        handle.addEventListener(evt, function (e) {
          ...
          dragElmt = function dragElmt(e) {
            ...
            if (!dragstarted) {
              ...
              elmt.style.opacity = opts.opacity; // if configured restore panel size to values before snap and reposition reasonable before drag actually starts
          ...
          if (dragstarted) {
            ...
            if (opts.snap) {
              ...
              if (opts.snap.callback && elmt.snappableTo && typeof opts.snap.callback === 'function') {
                ...
                if (opts.snap.repositionOnSnap && opts.snap[camelcase(elmt.snappableTo)] !== false) {
  ...
  fetch: function (_fetch) {
    ...
    var confDefaults = {
      ...
      autoreposition: true,
        ...
        conf = typeof obj.options.contentFetch === 'string' ? Object.assign({
    ...
    fetch(conf.resource, fetchInit).then(function (response) {
      ...
      conf.done.call(obj, obj, response); // extract and eval content of script tags if "evalscripttags"
  ...
  create: function create() {
    ...
    var closeBtn = self.querySelector('.jsPanel-btn-close'),
        ...
        minBtn = self.querySelector('.jsPanel-btn-minimize'); // needed only to fix an issue with pointer events in Firefox
    ...
    self.createMinimizedReplacement = function () {
      ...
      self.setIconfont(font, tpl); // needed only to fix an issue with pointer events in Firefox
    ...
    self.setHeaderTitle = function (hdrTitle, cb) {
      ...
      if (typeof hdrTitle === 'string') {
        titles.forEach(function (item) {
          ...
          // !! ALERT ¡¡ MODIFICACIÓN PARA QUE CAMBIE EL IDIOMA
          item.innerHTML = "<span data-i18n='"+hdrTitle+"'>"+i18n.t(hdrTitle)+"</span>";
          //////////////////////////////////////////////////////////////////
        });
      ...
      if (autoclose.progressbar) {
        ...
        if (autoclose.background) {
          if (jsPanel.themes.indexOf(autoclose.background) > -1) {
            ...
            self.autocloseProgressbar.style.background = '#' + jsPanel.colorNames[autoclose.background];
          } else {
            self.autocloseProgressbar.style.background = autoclose.background;
          }
        } else {
          self.autocloseProgressbar.classList.add('success-bg'); // default background for progressbar
        }
      }
    ...
    if (options.onwindowresize) {
      var onResize = options.onwindowresize; // if container is 'window'
      ...
      if (parentPanel) {
        ...
        document.addEventListener('jspanelresize', function (e) {
          ...
          if (e.panel === parentPanel) {
            ...
            } else if (status === 'smallifiedmax' && _onResize) {
              self.maximize().smallify();
            }
          }
        }, false);
